...
6. 每次发版必须按根目录 `cloudfunctions-manifest.json` 的清单部署全部云函数，不能只上传小程序前端。客户端会校验 `login` 返回的 `apiVersion`，版本不一致时会明确提示重新部署，避免线上前后端错配
...
4. 免费环境可继续使用“仅创建者可读写”，饭搭子的头像和菜谱图片由 `recipe` 云函数换取临时访问地址；无需为此升级套餐
5. `storage.rules` 仅作为付费套餐启用自定义安全规则后的可选配置，不是免费环境的部署必需项
6. 修改图片访问逻辑后，重新上传小程序代码，并重新部署 `recipe`、`user`、`feedback` 云函数
...
`app_config/family` 是管理员身份的唯一配置来源，必须在首次登录前创建并填写 `adminOpenid`。系统不再使用前端或云函数中的硬编码 OpenID；配置缺失时登录会直接提示修复，避免误把普通用户识别为主管理员。
...
1. 确认 `app_config/family`、数据库索引和 `database.rules.json` 已在目标环境配置完成；免费环境保持云存储“仅创建者可读写”即可
2. 按 `cloudfunctions-manifest.json` 逐个选择“上传并部署：云端安装依赖”，部署完后再上传小程序前端
3. 在微信开发者工具中点击“上传”，填写版本号和项目备注
4. 用体验版完成一次登录、跨设备饭篮、提交投喂单、收藏、图片显示和消息分页回归测试
5. 在微信公众平台提交审核，审核通过后发布上线
